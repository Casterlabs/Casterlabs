...
<script>
	...
	export function processEvent(event) {
		...
		if (readMessagesAloud) {
			...
			switch (event.event_type) {
				case 'CHANNEL_POINTS': {
					message = t('chat.viewer.message.CHANNELPOINTS', {
				...
				case 'SUBSCRIPTION': {
					message = t(`chat.viewer.message.SUBSCRIPTION.${event.sub_type}`, {
						months: event.months,
				...
				case 'FOLLOW': {
					message = t('chat.viewer.message.FOLLOW', {
				...
				case 'RAID': {
					message = t('chat.viewer.message.RAID', {
